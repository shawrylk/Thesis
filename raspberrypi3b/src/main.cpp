...
int sendFunc (char *sendData, int sendLen)
{
    ...
    std::cout << "command: " << STMData.command << std::endl;
...
void testUART()
{
    ...
    while(1)
    {
        if (bpsUARTReceiveData(&recvData, sizeof(bpsUARTSendDataTypeDef)) == BPS_ERROR)
            continue;
        std:: cout << "recv: " << recvData.command << std::endl;
